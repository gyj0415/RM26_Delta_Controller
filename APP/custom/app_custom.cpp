...
Joint joint1(std::make_unique<MotorController>(std::make_unique<DJIMotor>(
                 "joint1", DJIMotor::GM6020, (DJIMotor::Param){ 0x01, E_CAN1, DJIMotor::CURRENT })),
             2138,
             160,
             -160,
             0,
...
Joint joint2(std::make_unique<MotorController>(std::make_unique<DJIMotor>(
                 "joint2", DJIMotor::GM6020, (DJIMotor::Param){ 0x02, E_CAN1, DJIMotor::CURRENT })),
             3293,
             180,
             -180,
             1,
...
Joint joint3(std::make_unique<MotorController>(std::make_unique<DJIMotor>(
                 "joint3", DJIMotor::GM6020, (DJIMotor::Param){ 0x03, E_CAN1, DJIMotor::CURRENT })),
             2000,
             160,
             -160,
             0,
...
Algorithm::delta::Kinematics my_delta(100, 40, 100, 224);
Algorithm::delta::Dynamic my_dynamic(100, 40, 100, 224);

float num = 114;
void get_num(bsp_uart_e, uint8_t *s, uint16_t) {
    float target;
...
void app_custom_task(void *args) {
    ...
    while(true) {
        ...
        if(++c_count == 10) {
            c_count = 0;
            app_msg_vofa_send(E_UART_REFEREE, pos[0], pos[1], pos[2]);
        }
