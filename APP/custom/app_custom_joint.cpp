...
void Joint::joint_init() {
    if(motor_ctrl_) {
        motor_ctrl_->init();
        motor_ctrl_->add_controller(std::make_unique<Controller::MotorBasePID>(Controller::MotorBasePID::PID_SPEED |
                                                                                   Controller::MotorBasePID::PID_ANGLE,
                                                                               std::move(speed_pid_),
                                                                               std::move(angle_pid_),
                                                                               true));
        motor_ctrl_->add_controller(
            [&](const MotorController *) -> float { return static_cast<float>(t_tor * GM6020_TOR_TO_COUNT); },
