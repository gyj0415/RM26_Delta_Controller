...
public:
    Joint() = default;
    explicit Joint(std::unique_ptr<MotorController> motor_ctrl,
                   float zero_position,
                   float max_angle,
                   float min_angle,
                   float max_torque,
                   std::unique_ptr<Controller::PID> speed,
                   std::unique_ptr<Controller::PID> angle)
    : motor_ctrl_(std::move(motor_ctrl))
    , zero_position_(zero_position)
    , max_angle_(max_angle)
    , min_angle_(min_angle)
    , max_torque_(max_torque)
    , speed_pid_(std::move(speed))
    , angle_pid_(std::move(angle)) { }

    Joint(std::unique_ptr<MotorController> motor_ctrl,
          float zero_position,
          float max_angle,
          float min_angle,
          float max_torque,
          std::unique_ptr<Controller::PID> speed,
          std::unique_ptr<Controller::PID> angle,
          std::function<float(float)> feedforward_func)
    : motor_ctrl_(std::move(motor_ctrl))
    , zero_position_(zero_position)
    , max_angle_(max_angle)
    , min_angle_(min_angle)
    ...
    , speed_pid_(std::move(speed))
    , angle_pid_(std::move(angle))
    , feedforward_func_(std::move(feedforward_func)) { }
